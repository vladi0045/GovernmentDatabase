...
namespace DataLayer
{
    public class PoliticianContext : IDB<Politician, int>
    {
        ...
        public void Create(Politician item)
        {
            try
            {
                Party partyFromDB = _context.Parties.Find(item.Party.Id);
                ...
                Position positionFromDB = _context.Positions.Find(item.Position.Id);
        ...
        public Politician Read(int key, bool noTracking = false, bool useNavigationProperties = false)
        {
            try
            {
                IQueryable<Politician> query = _context.Politicians;
                ...
                Politician politicianFromDB = query.SingleOrDefault(a => a.Id == key);

                if (politicianFromDB == null)
                {
                    throw new ArgumentException("There is no politician with that key!");
                }

                return politicianFromDB;
        ...
        public Politician Find(int id)
        {
            try
            {
                return _context.Politicians.SingleOrDefault(i => i.Id == id);
            }
            catch (Exception ex)
            {
                throw ex;
