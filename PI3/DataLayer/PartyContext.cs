...
namespace DataLayer
{
    public class PartyContext : IDB<Party, int>
    {
        ...
        public void Create(Party item)
        {
            try
            {
                List<Politician> genres = new List<Politician>();

                foreach (var politician in item.Politicians)
                {
                    Politician politicianFromDB = _context.Politicians.Find(politician.Id);

                    if (politicianFromDB != null)
                    {
                        genres.Add(politicianFromDB);
                    }
                    else
                    {
                        genres.Add(politician);
                    }
                }

                item.Politicians = genres;
